...
class invite_tracker(commands.Cog):
    ...
    async def on_member_join(self, member):
        if int(member.guild.id) != int(Config.guild_id):
            return #Only for your server
        logs = self.bot.get_channel(int(self.logs_channel))
        eme = discord.Embed(description=f"Welcome member {len(list(member.guild.members))}th", color=0x03d692)
            ...
            for invite in invs_before:
                if invite.uses < self.find_invite_by_code(invs_after, invite.code).uses:
                    eme.add_field(name="Used invite",
                                  value=f"Invited by: {invite.inviter.mention} (`{invite.inviter}` | `{str(invite.inviter.id)}`)\nCode: `{invite.code}`\nUsed times: ` {str(invite.uses)} `", inline=False)
        except:
            pass
    ...
    async def on_member_remove(self, member):
        if int(member.guild.id) != int(Config.guild_id):
            return
        logs = self.bot.get_channel(int(self.logs_channel))
        eme = discord.Embed(description=f"Just leave, the server now has {len(list(member.guild.members))} members", color=0xff0000, title=" ")
        eme.set_author(name=str(member), icon_url=member.display_avatar.url)
        eme.set_footer(text="ID: " + str(member.id))
        ...
        try:
            ...
            for invite in invs_before:
                if invite.uses > self.find_invite_by_code(invs_after, invite.code).uses:
                    eme.add_field(name="Used invite",
                                  value=f"Invite by: {invite.inviter.mention} (`{invite.inviter}` | `{str(invite.inviter.id)}`)\nCode: `{invite.code}`\nUsed times: ` {str(invite.uses)} `", inline=False)
        except:
            pass
        await logs.send(embed=eme)
